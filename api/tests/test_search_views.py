...
from api.utils.constants import SEARCH_CACHE_KEY, TEST_SEARCH_USERS_RESPONSE, TEST_SEARCH_REPOSITORIES_RESPONSE
...
@pytest.mark.parametrize("search_type, service_response", [
    (SearchType.USERS.value, TEST_SEARCH_USERS_RESPONSE),
    (SearchType.REPOSITORIES.value, TEST_SEARCH_REPOSITORIES_RESPONSE),
])
def test_search_success_and_cache(search_type, service_response,
                                  api_client, service_layer_class):
...
def test_clear_endpoint_cache(api_client, django_user_model):
    ...
    user: User = django_user_model.objects.create_user(username="user", password="user", is_staff=False)
    ...
    admin: User = django_user_model.objects.create_user(username="admin", password="admin", is_staff=True)
