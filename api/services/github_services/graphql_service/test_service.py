...
from api.services.github_services.graphql_service.exceptions import GithubServiceException
...
def test_execute_request_success(mocker):
    fake_response = get_fake_response(json.dumps({'data': {"search": {"nodes": "result"}}}).encode())
...
def test_execute_request_graphql_error(mocker):
    fake_response = get_fake_response(json.dumps({'errors': ['Some problem with GraphQL']}).encode())
...
def test_service_end_to_end(tmp_path, mocker):
    ...
    fake_response = get_fake_response(json.dumps({'data': {"search": {"nodes": TEST_SEARCH_USERS_RESPONSE}}}).encode())
    mocker.patch('requests.post', return_value=fake_response)
    ...
    repositories_response = [{'id': 1, 'title': 'title', 'owner': {"login": "owner"}, 'stars': 12, 'description': None,
                              'url': 'http://github.com/repository_url'}]
    fake_response = get_fake_response(json.dumps({'data': {"search": {"nodes": repositories_response}}}).encode())
    mocker.patch('requests.post', return_value=fake_response)
